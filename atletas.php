...
<body>
    ...
    <div class="contenido_main" >
    	<?php
    		...
    		$result = mysqli_query($con, "SELECT Nombre_Arquero, Apellido_Arquero, fecha_de_nacimiento, Ranking from arquero;")or die("No se puede hace la consulta a la base de datos");
    		if (mysqli_num_rows($result) > 0){
    				while($row = mysqli_fetch_assoc($result)){
    					print('<h3 class="eventos">' . $row["Nombre_Arquero"] . " - " . $row["Apellido_Arquero"] . " - " . $row["fecha_de_nacimiento"] . " - " . $row["Ranking"] . "</h3>");
    				} 
    		} else {
    			printf("<p>No hay atletas todavia</p>");
    		}
    		mysqli_close($con);
    	?>
    </div>
